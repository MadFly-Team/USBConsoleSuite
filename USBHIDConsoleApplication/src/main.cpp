...
int           test();
unsigned char pathBlackstar[ 256 ];
...
#define MAX_STR 255
    ...
    std::cout << "How doing the AMP test.." << std::endl;
    test();
    ...
    handle = hid_open( 0x27D4, 0xCF, NULL );
    if ( !handle )
    {
    ...
    else
    {
        ...
        for ( i = 0; i < res; i++ ) printf( "%02x ", (unsigned int)buf[ i ] );
        printf( "\n" );
    }
    ...
    while ( res == 0 )
    {
        ...
        i++;
        if ( i >= 10 )
        { /* 10 tries by 500 ms - 5 seconds of waiting*/
            printf( "read() timeout\n" );
            break;
        }
...
int test()
{
    static unsigned char hidMsg[ 64 ];
    ...
    std::cout << "HIDAPI Test" << std::endl;
    std::cout << "===========" << std::endl;
    std::cout << "Opening the USB connection to HID 27D4-00CF" << std::endl;

    // Open the USB HID device (change VID and PID based on your device)
    hid_device* handle = hid_open( 0x27D4, 0xCF, nullptr );
    ...
    std::cout << "Size of hidMsg: " << sizeof( hidMsg ) << std::endl;
    int bytesWritten2 = hid_write( handle, hidMsg, sizeof( hidMsg ) );
    if ( bytesWritten2 < 0 )
    {
        std::cerr << "Error writing to HID device." << std::endl;
    }
    std::cout << "Wrote " << bytesWritten2 << " bytes." << std::endl;

    // Set the hid_read() function to be non-blocking.
    hid_set_nonblocking( handle, 1 );
    Sleep( 1000 );

    std::cout << "Conneted, sending handshaket HID 27D4-00CF" << std::endl;
    ...
    // Write data to the HID device
    hidMsg[ 1 ]      = 0x04;
    int bytesWritten = hid_write( handle, hidMsg, sizeof( hidMsg ) );
    if ( bytesWritten < 0 )
    {
        std::cerr << "Error writing to HID device." << std::endl;
    }
    ...
    std::cout << "Conneted, sending handshaket HID 27D4-00CF" << std::endl;
    // Read data from the HID device
    bytesRead = hid_read( handle, buffer, sizeof( buffer ) );
    ...
    Sleep( 4000 );
    hid_close( handle );
    std::cout << "End test of Message" << std::endl;
    return 0;
}
